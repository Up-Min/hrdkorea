...
public class VacController extends HttpServlet {
	...
	protected void doIt(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case "/look" : site="look.jsp"; break;
		case "/watch" : site= vc.watch(request, response); break;
		case "/calc" : site = vc.calc(request, response); System.out.println("cont"); break;		
		case "/wrong" : site="wrong.jsp"; break;		
		case "/insert" : int result = vc.insert(request,response); 
				...
				if(result == 1) {
					...
					out.print("<script>");
					out.print("alert('등록 실패!'); location.href='" + context + "';");
					out.print("</script>");
					out.flush();
				}break;
